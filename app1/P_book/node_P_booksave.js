...
var upload=multer({dest:'./P_book/img'}) //此步是配置上传文件的存放位置，然后在路由过程中调用upload下面的一些方法.
...
function ajax(app1){
	app1.post('/ajax/book/admin/save',upload.single('posters_file'),function(req,res){
		var nav_bookId=req.body.nav  //ajax从页面中传递过来的 用来判断是否更新的条件值 用Id 来表示。
		
		var id=req.body.id
		var image=req.body.image  //没有文件上传的话就使用外链路径(input输入框中的text信息)		
/*		if(req.file){ //不能用req.file.fieldname  因为有二级对象调用，框架不支持检索二级对象层.
		...
		if(nav_bookId!=''){ //如果里面有id ，则把_id 值加入到doc中，以便覆盖原数据内容，否则就不加_id  那就是新建一条数据了。
